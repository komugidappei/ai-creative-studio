  ...
  try {
    ...
    if (isFreePlan && monthlyGenerations >= freeLimit) {
      return NextResponse.json({ 
        error: 'Monthly limit reached. Please upgrade to continue generating videos.',
        limit: freeLimit,
    ...
    try {
      switch (provider) {
        case 'replicate':
          result = await generateVideoWithReplicate({ prompt })
          break
        default:
          result = await generateMockVideo({ prompt })
      }
